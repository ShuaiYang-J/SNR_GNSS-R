...
# from astropy.timeseries import LombSacrgle # from astropy V3  暂时没有
import numpy
import math
from math import pi
...
figPath='C:/Data/SNR_SanXia/out/'
#INPUT FILE
# archivo='C:/Data/input1/SNR-GPS.txt'#Input file
archivo='C:/Data/SNR_SanXia/SNR-GPS.txt'

##PARAMETERS
#观测值的间隔数 s 秒
int_tiempo=1
# 判定是否为同一个轨迹 就是间隔不超过35s 
epocas_offset=35 ##35 for 1 second interval, 7 for a 5 seconds interval and 1.1-1.9 for 30 seconds interval  
...
# 
#生成output文件的表头 所有卫星在一个文件内
# salida = open(figPath+str(sat.strip())+'/'+'final_values.txt', 'w') 
salida = open(figPath+'final_values.txt', 'w') 
#Header line
salida.write('epoch good_track index_good epoch_time month day id_sat Azi_ini aZi_end ele_ini ele_end A desvA Phi desvPhi ref_height SatelliteDirection\n') 
salida2 = open(figPath+'final_values_good.txt', 'w') 
#Header line
        ...
        if sat in datos[2] and float(datos[4])>el_min and float(datos[4])<el_max and float(datos[5])>azi_min and float(datos[5])<azi_max: 
            ...
            try:
                ...
                if (e-e2)<epocas_offset:
                    ...
                    el=float(datos[4])
                    SNR=float(datos[3])
                    e1=float(datos[0]) #历元数
                    h=str(datos[1]) #历元内小时数
                    anio=datos[-3]
                    mes=datos[-2]
                    dia=datos[-1].strip()
                
                    SNR_v=10**(SNR/20) # SNR in volts 由dB转为v/v
    ...
    for i in range(0,lon): # 每一个有效轨迹进行处理
        ...
        for j in range(len(el_lis)): # for every element of the list 每一个高度角
           # apply the computed coefficients 利用对观测数据的拟合参数 创造一个SNR_cal 二次 一次 常数
            ...
            SNRi_cal.append(SNR_cal) #计算值
            
        SNRi_lis2.append(SNRi_lis) #反射信号大小 v/v
       ...
       # 1o iterations in the adjustment 10次迭代
        ...
        # with the same dimention as the track we are working on 时间间隔 ts 记录数据持续时间
        for k in range(len(el_lis)):
            ts=contador*int_tiempo
            ts_lis.append(ts)
            contador=contador+1
            
        # 处理观测时长半小时以上且 高度角范围大于10的（inc_eleva)
        el_lis_sen=[]    
        if len(el_lis)>(30*60/int_tiempo) and (max(el_lis)-min(el_lis))>inc_eleva:
        # if  (max(el_lis)-min(el_lis))>inc_eleva:
            ...
            for i in frec_sen:                
            #   最大高度限制emmmm  2.5   
                if i*land/2 <8: ##maximum H allowed is of 2.5 meters  最大高度限制 大于两倍的lamd 最少
                    frec2_sen.append(i*land/2)
                    line_sec=(power_sen[aux]) #？？？？aux是每一个值加1 
        ...
        #    ？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？依据是什么
            ...
            font_size=8
            nombre_dib=epoch+'-'+str(fechi.strip())+'-'+str(h_lis[0])+'-'+str(h_lis[-1])+'-'+str(sat.strip())+'-'+str(primer_az)+'-'+str(final_az)
